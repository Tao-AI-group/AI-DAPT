...
def my_collate(batch):
    if multilabel_m : 
        mb_t, lbt_t, seq_l, mtd = preprocess_multilabel(batch, pack_pad, half_m) ### LR Sep 30 20 added surv_m
    else: 
        mb_t, lbt_t, seq_l, mtd = preprocess(batch, pack_pad, surv_m, half_m) ### LR Sep 30 20 added surv_m
...
#customized parts for EHRdataloader
def my_collate(batch):
    mb_t, lbt_t,seq_l, mtd =preprocess(batch,pack_pad,surv_m) ### LR Sep 30 20 added surv_m
    return [mb_t, lbt_t,seq_l, mtd]
...
class EHRdataloader(DataLoader):
    def __init__(self, dataset, batch_size=128, shuffle=False, sampler=None, batch_sampler=None,
                 ...
                 timeout=0, worker_init_fn=None, packPadMode = False , surv=False): ### LR Sep 30 20 added surv
