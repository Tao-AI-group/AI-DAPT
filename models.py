...
class EHR_RNN(EHREmbeddings):
    def __init__(self, input_size, embed_dim, hidden_size, n_layers=1, dropout_r=0.1, cell_type='GRU', bii=False, time=False, preTrainEmb='', packPadMode = True, surv=False, cls_dim=1):
       	EHREmbeddings.__init__(self, input_size, embed_dim, hidden_size, n_layers=n_layers, dropout_r=dropout_r, cell_type=cell_type, bii=bii, time=time, preTrainEmb=preTrainEmb, packPadMode=packPadMode, surv=surv, cls_dim=cls_dim)
...
class RETAIN(EHREmbeddings):
    def __init__(self, input_size, embed_dim, hidden_size, n_layers):        
        ...
        self.RNN1 = nn.GRU(embed_dim, hidden_size, 1, batch_first=True, bidirectional=True, dropout=0.1)
        self.RNN2 = nn.GRU(embed_dim, hidden_size, 1, batch_first=True, bidirectional=True, dropout=0.1)

        self.wa = nn.Linear(hidden_size*2, 1, bias=False)
        # self.Wb = nn.Linear(hidden_size*2, hidden_size, bias=False)
        self.Wb = nn.Linear(hidden_size*2, self.embed_dim, bias=False)
